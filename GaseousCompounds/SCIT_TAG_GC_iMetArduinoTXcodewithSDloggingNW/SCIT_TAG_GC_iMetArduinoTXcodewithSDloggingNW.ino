...
void printDateTime(); //prints date/time to the iMet
void printDiff(long); //helps with the format of printing to the imet
...
#define greenLEDpin 3
 ...
 RTC_PCF8523 RTC; //this one works, the above does not
...
//this initializes time to current time
  if (! RTC.initialized()) {
...
#if ECHO_TO_SERIAL
//Will need to comment out all spaces printed to console during flight and uncomment print("FF") which is the delimeter
  Serial.println("New Series");
  char buff[4];
  sprintf(buff, "%04d", frameCount); //prints frame number to serial output
  Serial.print(buff); 
  Serial.print(' ');
  //Serial.print("FF"); //comment out the  ' ' and replace it with this
...
//these functions are used to read and calculate differentials
  ...
  */
  ...
  logfile.println("/n");
       ...
       //Serial.print(" "); 
        ...
        //Serial.print(" ");
        //Serial.println("Begin differentials.");  //this outputs to the serial monitor. Turn this off for flight. Add this to ground code

        //these print differentials to serial monitor
        Serial.print("adc01:");
        printDiff(adc01); //PT
        Serial.print("adc23:");
        printDiff(adc23); //SN1
        ...
        Serial.print("adc89:");
        printDiff(adc89);
        Serial.print("adc1011:");
        printDiff(adc1011);
        Serial.print("adc1213:");
        printDiff(adc1213);
        Serial.print("adc1415:");
        printDiff(adc1415);
...
void printDateTime()
{
   ...
   DateTime now = RTC.now();
      ...
      char Date[8];
      char Time[8];
   sprintf(Date, "%4d%02d%02d", Year, Month, Day); //prints date
   Serial.print("Date:");
   Serial.print(Date);
   Serial.print(" ");
   sprintf(Time, "%02d%02d%02d", Hour, Minute, Second); //converts time to even output
   Serial.print("Time:");
   Serial.print(Time);
   Serial.print(" ");
} //https://forum.arduino.cc/t/get-year-month-etc-from-rtc-ds1307-individually/503083

void printDiff(long adc) //writen by me
{
  //Will need to comment out all spaces printed to console during flight
  if (adc < 0) //if negative
  {
    Serial.print("2D");
    adc = -1 * adc;
  }
  String num = String(adc);
  if (num.length()%2 != 0)
  {
    Serial.print("0");
  }
